...
import { ShieldCheck, LayoutDashboard, Inbox, Clock, CheckCircle, LogOut, Shield } from 'lucide-react';
...
export const OfficerLayout: React.FC = () => {
  ...
  const navItems = [
    { label: 'Officer Dashboard', path: '/officer', icon: LayoutDashboard, exact: true },
    { label: 'Applications', path: '/officer/applications', icon: Inbox },
    { label: 'Pending Review', path: '/officer/pending', icon: Clock },
    { label: 'Approved', path: '/officer/approved', icon: CheckCircle },
  ];
    ...
    <div className="min-h-screen flex bg-background text-text-main">
      <aside className="w-64 bg-card border-r border-border hidden md:flex flex-col">
        <div className="h-16 flex items-center px-6 border-b border-border">
          <Link to="/officer" className="flex items-center gap-2 text-accent-green hover:text-[#9BE600] transition-colors">
        ...
        <header className="h-16 bg-card border-b border-border flex items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex-1 flex justify-end items-center gap-4">
            <div className="flex items-center gap-2 text-sm text-text-muted border-r border-border pr-4">
              ...
              <span className="hidden sm:inline-block font-medium text-text-main">Review Officer</span>
            </div>
            <button 
