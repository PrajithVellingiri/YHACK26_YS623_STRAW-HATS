...
import { MOCK_APPLICATIONS, MOCK_DEPARTMENTS } from '../data/mockData';
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h1 className="text-2xl font-bold text-gov-navy mb-2">Master Applications Overview</h1>
        <p className="text-slate-500">Monitor all applications across all departments and statuses.</p>
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">Business Info</th>
                <th className="px-6 py-4 font-bold">Compliance & Dept</th>
              ...
              {filteredApps.length > 0 ? (
                filteredApps.map((app, idx) => (
                  <tr key={app.id} className={`border-b border-slate-100 hover:bg-slate-50/80 transition-colors ${idx === filteredApps.length - 1 ? 'border-none' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-500">{app.industry} • {app.applicant}</div>
                    </td>
                    <td className="px-6 py-4">
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                    ...
                    <p className="font-medium text-slate-600">No applications match your filters.</p>
                  </td>
                </tr>
              )}
            </tbody>
